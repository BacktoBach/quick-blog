    ...
    <header className="sticky top-0 z-50 border-b border-gray-100 bg-white/90 backdrop-blur-md dark:border-gray-800 dark:bg-gray-950/90">
      <div className="mx-auto flex h-[74px] max-w-7xl items-center justify-between px-6">
        ...
        <nav className="flex items-center gap-5">
          ...
          {isAuthenticated && <NavLink to="/my-posts" className={linkClass}>My Posts</NavLink>}
          {isAuthenticated && <NavLink to="/posts/new" className={linkClass}>New Post</NavLink>}
          ...
          {isAuthenticated ? (
            ...
            <Link to="/login" className="rounded-full bg-indigo-600 px-7 py-2.5 text-sm font-semibold text-white">
              Login
            </Link>
          )}
        </nav>
