...
export default function CreatePost() {
  ...
  return (
    <section className="mx-auto max-w-4xl space-y-6 py-8">
      <div>
        <h1 className="text-3xl font-extrabold text-gray-950 dark:text-white">Create new post</h1>
        <p className="mt-1 text-sm text-gray-500">
          TinyMCE is self-hosted locally. Cover images upload to Cloudinary when the unsigned preset is configured.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <label className="block">
            ...
            <select
              ...
              className="mt-1 w-full rounded-lg border border-gray-200 bg-transparent px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-indigo-500 dark:border-gray-800"
            >
              {['Technology', 'Startup', 'Lifestyle', 'Finance'].map((category) => (
        ...
        <label className="block">
          <span className="flex items-center justify-between gap-3 text-sm font-medium text-gray-700 dark:text-gray-300">
            ...
            >
              {cloudinaryReady ? 'Cloudinary upload enabled' : 'Local preview mode'}
            </span>
          </span>
          <input
            type="file"
        ...
        {preview && <img src={preview} alt="Cover preview" className="aspect-[21/8] w-full rounded-lg object-cover" />}
        ...
        >
          {loading ? (cloudinaryReady ? 'Uploading image...' : 'Publishing...') : 'Publish post'}
        </button>
      </form>
    </section>
  );
